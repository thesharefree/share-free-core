...
import { UserAnswer, UserAnswerDocument } from 'src/entities/user-answer.entity';
...
export class UserAnswerService {
  ...
  public async updateAnswer(queryId: string, answer: string, loggedInUser: string): Promise<void> {
    const user = await this.userModel.findOne({ email: loggedInUser });
    const query = await this.queryModel.findById(queryId);
    if (query == null) {
      throw new HttpException('Invalid query', 400);
    } else {
      let userAnswer = await this.userAnswerModel.findOne({ userId: user._id, queryId: queryId });
      if (userAnswer != null) {
        this.userAnswerModel.updateOne({ _id: userAnswer._id }, {
          answer: answer,
  ...
  public async userAnswers(email: string): Promise<UserAnswer[]> {
    ...
    if (user == null) {
      ...
      const userAnswers = await this.userAnswerModel.find({ userId: user._id }).lean();
      const answeredQueryIds = userAnswers.map(userAnswer => { return userAnswer.queryId; });
  ...
  public async pendingQueries(loggedInUser: string): Promise<Query[]> {
    ...
    if (user == null) {
      ...
      const answeredQueryIds = userAnswers.map(answer => { return answer.queryId });
      ...
      return (await this.queryModel.find().where('_id').in(pendingQueryIds)).filter(query => query.target.includes('USER'));
    }
  }
