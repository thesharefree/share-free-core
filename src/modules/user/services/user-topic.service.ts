...
import { UserTopicXref, UserTopicXrefDocument } from 'src/entities/user-topic-xref.entity';
...
export class UserTopicService {
  ...
  constructor(
    ...
    @InjectModel(UserTopicXref.name) private readonly userTopicXrefModel: Model<UserTopicXrefDocument>,
  ) { }

  public async assignTopics(topicIds: string, loggedInUser: string): Promise<void> {
    const user = await this.userModel.findOne({ email: loggedInUser });
    await this.userTopicXrefModel.deleteMany({ userId: user._id });
    topicIds.split(',').forEach(async topicId => {
      const topic = await this.topicModel.findById(topicId);
      if (topic != null) {
        const xrefResp = await this.userTopicXrefModel.findOne({ userId: user._id, topicId: topicId });
        if (xrefResp == null) {
  ...
  private newUserTopicXref(userId: string, topicId: string, loggedInUser: string): UserTopicXref {
  ...
  public async getUserTopics(loggedInUser: string): Promise<Topic[]> {
    ...
    const topicIds = xrefResps.map(xref => { return xref.topicId; });
