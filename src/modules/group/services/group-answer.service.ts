...
export class GroupAnswerService {
  constructor(
    ...
    @InjectModel(GroupAnswer.name)
    private readonly groupAnswerModel: Model<GroupAnswerDocument>,
  ...
  public async updateAnswer(
    ...
    if (group == null) {
      ...
      if (query == null) {
        ...
        let groupAnswer = await this.groupAnswerModel.findOne({
          groupId: group._id,
          queryId: queryId,
        });
        if (groupAnswer != null) {
  ...
  public async groupAnswers(groupId: string): Promise<GroupAnswer[]> {
    ...
    if (group == null) {
      ...
      const groupAnswers = await this.groupAnswerModel
        .find({ groupId: group._id })
        .lean();
      const answeredQueryIds = groupAnswers.map((groupAnswer) => {
        return groupAnswer.queryId;
      });
  ...
  public async pendingQueries(groupId: string): Promise<Query[]> {
    ...
    if (group == null) {
      ...
      const groupAnswers = await this.groupAnswerModel.find({
        groupId: group._id,
      });
      const answeredQueryIds = groupAnswers.map((answer) => {
      ...
      ).filter((query) => query.target.includes('GROUP'));
    }
  }
  ...
  public async deleteAnswer(groupId: string, queryId: string): Promise<void> {
    ...
    if (group == null) {
      ...
      await this.groupAnswerModel.deleteOne({
        groupId: group._id,
        queryId: queryId,
      });
    }
