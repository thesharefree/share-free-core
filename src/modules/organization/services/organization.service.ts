...
export class OrganizationService {
  ...
  public async updateOrganization(
    ...
    const extOrganization = await this.organizationModel.findById(
      organizationId,
    );
    ...
    if (
      extOrganization.owner !== loggedInUser.email &&
  ...
  public async uploadBanner(
    ...
    file = {
      ...file,
      originalname:
        'org/images/' + extOrganization._id.toString() + '.' + extension,
    };
    const storageUrl = await this.azureStorage.upload(file);
    console.log(JSON.stringify(storageUrl));
    await this.organizationModel.updateOne(
      { _id: extOrganization._id },
      {
        banner: storageUrl,
        updatedBy: loggedInUser.email,
        updatedDate: new Date(),
