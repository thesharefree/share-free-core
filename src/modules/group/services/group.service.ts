...
export class GroupService {
  ...
  public async updateGroup(groupId: string, group: Group, loggedInUser: string): Promise<void> {
    ...
    await this.groupModel.updateOne({ _id: groupId }, {
      name: group.name,
  ...
  public async uploadBanner(file: UploadedFileMetadata, groupId: string, loggedInUser: string): Promise<void> {
    const group = await this.groupModel.findById(groupId);
    ...
    await this.groupModel.updateOne({ _id: group._id }, {
      banner: storageUrl,
      updatedBy: loggedInUser,
      updatedDate: new Date()
    });
  ...
  public async conferenceToken(groupId: string, loggedInUser: string): Promise<string> {
    const extGroup = await this.groupModel.findById(groupId);
    if (extGroup == null) {
      throw new HttpException('Invalid Group', 400);
    }
    ...
    const rtcToken = RtcTokenBuilder.buildTokenWithUid(appID, appCertificate, extGroup._id.toString(), 0, role, privilegeExpiredTs);
    await this.groupModel.updateOne({ _id: groupId }, {
      rtcToken: rtcToken,
      rtcExpiry: rtcExpiry
    });
  ...
  public async report(groupId: string, category: string, loggedInUser: string): Promise<void> {
    const extGroup = await this.groupModel.findById(groupId);
    if (extGroup == null) {
      throw new HttpException('Invalid Group', 400);
    }
