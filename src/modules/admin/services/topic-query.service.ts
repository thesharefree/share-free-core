...
export class TopicQueryService {
  ...
  public async toggleQueryXref(
    ...
    if (topic == null) {
      ...
      if (query == null) {
        ...
        const xrefResp = await this.topicQueryXrefModel.findOne({
          topicId: topicId,
          queryId: queryId,
        });
        if (xrefResp != null) {
          await this.topicQueryXrefModel.findOneAndDelete({
            topicId: topicId,
            queryId: queryId,
          });
        } else {
  ...
  public async getTopicQueries(topicId: string): Promise<Query[]> {
    ...
    const queryXrefList = await this.topicQueryXrefModel.find({
      topicId: topicId,
    });
    const finalQueryList = [];
    queryList.forEach((query) => {
      const xrefExist = queryXrefList.filter(
        (xref) => xref.queryId == query._id,
      );
