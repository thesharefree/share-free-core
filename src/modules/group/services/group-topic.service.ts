...
export class GroupTopicService {
  ...
  public async assignTopics(groupId: string, topicIds: string, loggedInUser: string): Promise<void> {
    ...
    if (group == null) {
      ...
      topicIds.split(',').forEach(async topicId => {
        ...
        if (topic != null) {
          const xrefResp = await this.groupTopicXrefModel.findOne({ groupId: group._id, topicId: topicId });
          if (xrefResp == null) {
  ...
  private newGroupTopicXref(groupId: string, topicId: string, loggedInUser: string): GroupTopicXref {
  ...
  public async getGroupTopics(groupId: string): Promise<Topic[]> {
    ...
    if (group == null) {
      ...
      const xrefResps = await this.groupTopicXrefModel.find({ groupId: group._id });
