...
import { UserGroupRequestXref, UserGroupRequestXrefDocument } from 'src/entities/user-group-request-xref.entity';
...
export class GroupUserService {
  ...
  constructor(
    ...
    @InjectModel(UserGroupRequestXref.name) private readonly userGroupRequestXrefModel: Model<UserGroupRequestXrefDocument>,
  ) { }
  ...
  public async askToJoin(groupId: string, loggedInUser: string): Promise<void> {
    ...
    if (group != null) {
      const xrefResp = await this.userGroupRequestXrefModel.findOne({ userId: user._id, groupId: groupId });
      if (xrefResp != null) {
        throw new HttpException('Request already exists', 400);
      } else {
        const xrefResp = await this.userGroupXrefModel.findOne({ userId: user._id, groupId: groupId });
        if (xrefResp != null) {
          ...
          const createdUserGroupRequestXref = new this.userGroupRequestXrefModel(xref);
  ...
  public async revokeRequest(groupId: string, loggedInUser: string): Promise<void> {
    const user = await this.userModel.findOne({ email: loggedInUser });
    const group = await this.groupModel.findById(groupId);
    if (group != null) {
      await this.userGroupRequestXrefModel.deleteOne({ groupId: group._id, userId: user._id });
    }
  }
  ...
  public async pendingUsers(groupId: string): Promise<User[]> {
    const group = await this.groupModel.findById(groupId);
    if (group != null) {
      const xrefResp = await this.userGroupRequestXrefModel.find({ groupId: groupId });
      const userIds = xrefResp.map(xref => { return xref.userId; });
  ...
  public async addToGroup(userId: string, groupId: string, loggedInUser: string): Promise<void> {
    ...
    if (user == null) {
      ...
      if (group == null) {
        ...
        if (group.owner != loggedInUser) {
          ...
          const xrefResp = await this.userGroupXrefModel.findOne({ userId: userId, groupId: groupId });
          if (xrefResp != null) {
            throw new HttpException('User is already a member', 400);
          } else {
            var usersCount = await this.userGroupXrefModel.find({ groupId: groupId }).count();
            if (usersCount >= 15) {
  ...
  private newUserGroupRequestXref(userId: string, groupId: string, loggedInUser: string): UserGroupRequestXref {
  ...
  private newUserGroupXref(userId: string, groupId: string, loggedInUser: string): UserGroupXref {
  ...
  public async removeFromGroup(userId: string, groupId: string, loggedInUser: string): Promise<void> {
    const user = await this.userModel.findById(userId);
    if (user == null) {
      throw new HttpException('Invalid user', 400);
    } else {
      ...
      if (group == null) {
        ...
        if (group.owner != loggedInUser) {
          ...
          await this.userGroupXrefModel.deleteOne({ userId: userId, groupId: groupId });
        }
      }
    }
  }

  public async leaveGroup(groupId: string, loggedInUser: string): Promise<void> {
    ...
    if (group == null) {
      ...
      if (group.owner != loggedInUser) {
        ...
        await this.userGroupXrefModel.deleteOne({ userId: user._id, groupId: groupId });
      }
    }
  }
